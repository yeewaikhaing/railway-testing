...
import { default as MedusaTotalsService} from "@medusajs/medusa/dist/services/totals";
...
type MyAllocationMapOptions = {
    exclude_gift_cards?: boolean
    exclude_discounts?: boolean
  }
  
type MyCalculationContextOptions = {
    is_return?: boolean
...
export class TotalsService extends MedusaTotalsService {
    ...
    private readonly container: TotalsServiceProps;

    constructor(container: TotalsServiceProps, private readonly config: any) {
        super(container);
  ...
  async getSubtotal(
    ...
    const getLineItemSubtotal = async (item: LineItem): Promise<number> => {
      const totals = await this.myGetLineItemTotals(item, cartOrOrder, {
        include_tax: true,
  ...
  async getCalculationContext(
    cartOrOrder: Cart | Order,
    options: MyCalculationContextOptions = {}
   ...
   */
   async getAllocationMap(
    ...
    options: MyAllocationMapOptions = {}
   ...
   */
   async myGetLineItemTotals(
    lineItem: LineItem,
    cartOrOrder: Cart | Order,
    options: MyLineItemTotalsOptions = {}
  ): Promise<MyLineItemTotals> {
    const calculationContext =
    ...
    const lineItemTotals: MyLineItemTotals = {
   ...
   * @returns An object that breaks down the totals for the shipping method
   */
   async myGetShippingMethodTotals(
    shippingMethod: ShippingMethod,
    cartOrOrder: Cart | Order,
    opts: MyGetShippingMethodTotalsOptions = {}
  ): Promise<MyShippingMethodTotals> {
